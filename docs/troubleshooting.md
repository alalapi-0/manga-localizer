...
access. In Safari, Add to Home Screen uses the local companion icon; that is not an App Store
install.
...
working without opening the batch drawer. Use **查看队列** on the failure or queued notice to open the
matching job, or **批处理与导出** to watch the whole queue. Details
