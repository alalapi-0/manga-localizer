...
export function capabilitiesFixture(): AppCapabilities {
  return {
    providers: [
      ...
      { id: 'ppocr-v3+tesseract', label: 'PP-OCRv3 + Tesseract 联合检测', kind: 'detector', available: true, local: true, isMock: false },
...
export function seedWorkbench(options: {
  ...
  useWorkbenchStore.setState({
    ...
    g4Contexts: Object.fromEntries(images.map((image) => [image.id, {
